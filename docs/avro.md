    ...
    there is a tool to convert IDL to JSON. nice! it generates a slightly 
      different JSON than the one needed to generate classes and has to be 
      tweaked a little. buuh! this cumbersome again, but still much better than
      writing the JSON schemata from scratch
    ...
    maintenance and updates of schemas can be done in 2 steps 
      - editing the JSON schemas  directly
      - autogenerating new classes from them
      this spares generating and tweaking the JSON RPC intermediate formats
      it is however a pity if the IDL get out of sync since they are the most 
        succinct description of the format
...
Avro specific mapping requires the autogeneration of classes from the schema 
whereas generic mapping works from the schema directly. Generic mapping though 
doesn't provide static type checking.
Therfor, as much as we'd love to work with the schema directly, without any 
...
datastructure before it is written. 
  ...
  Constructor
    ...
    - doesn't provide defaults
    - fields don't need to be set explicitely if a default value is given (?)
    - allows if/else constructs when constructing the record (killer pro argument!)
    - is more performant
